...
public:
    ...
    OCRepresentation m_motorRep;
    ObservationIds m_interestedObservers;

public:
    /// Constructor
    MotorResource()
      :m_name("Motor 1"), m_enc(0),m_vel(0),m_temp(0),m_vel_des(0),m_enc_des(0), m_Uri("/a/motor") {
        // Initialize representation
        m_motorRep.setUri(m_Uri);

        m_motorRep.setValue("state", m_state);
        m_motorRep.setValue("power", m_power);
        m_motorRep.setValue("name", m_name);
    }
    ...
    void createResource()
    {
        std::string resourceURI = m_Uri; //URI of the resource
        std::string resourceTypeName = "core.motor"; //resource type name. In this case, it is light
        ...
        EntityHandler cb = std::bind(&MotorResource::entityHandler, this,PH::_1);
...
void * ChangeLightRepresentation (void *param)
{
    MotorResource* lightPtr = (MotorResource*) param;
...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        // (in this case instance of class 'MotorResource').
        MotorResource myLight;
